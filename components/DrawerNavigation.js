...
const DrawerNavigation = () => {
  ...
  return (
   <>
      <button className="sm:hidden absolute top-0 right-0 px-8 py-4 z-40" onClick={openDrawer}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#2f847c" className="h-8 w-8">
          <path d="M3,6H21V8H3V6M3,11H21V13H3V11M3,16H21V18H3V16Z" />
        </svg>
      </button>
      <div className={`sm:hidden absolute shadow-lg z-50 bg-zinc-50 h-full w-3/4 px-8 py-4 flex flex-col gap-6 -top-0 -right-3/4 ease-in-out transition-transform ${isDrawerOpen? '-translate-x-full': ''}`}>
