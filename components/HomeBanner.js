...
const SKILLS_DATA = [
  {
    ...
    animation: 'motion-reduce:animate-none animate-floating',
...
const HomeBanner = () => {
  ...
  return (
    <div className="flex-1 w-full grid gap-0 grid-cols-2 grid-rows-1">
      <div className="text-celadon-green text-left font-[Lekton] flex flex-col justify-center">
        <span className="text-7xl w-fit bg-zinc-50 p-6">
          <i>Hello!</i> I am
        </span>
        <span className="text-7xl w-fit bg-zinc-50 p-6 font-bold">
          JD Hernandez
        </span>
        <span className="text-4xl w-fit bg-zinc-50 p-6 mt-8">
          I have around 8 years of experience in making software using JavaScript technologies!
        </span>
        <span className="w-fit bg-zinc-50 p-6 mt-8 flex flex-row gap-4 text-center">
          <span
            key="counter"
            className="w-10 h-10 text-center relative cursor-pointer select-none inline-flex items-center rounded-full bg-celadon-green px-2 text-2xl text-white"
            data-tooltip="Find and click the corresponding icons!"
          >
            {clicked.length}
          </span>
          {
      ...
      <div className="flex flex-col justify-center">
        <div className="relative h-fit w-fit mx-auto p-104">
          <img
            className="select-none mx-auto w-96 h-96 rounded-full border-8 border-zinc-50"
          ...
          />
          {
            SKILLS_DATA.map(({ animation, name, position, icon }) => (
              <span
                key={`${name}-${animation}}`}
                className={`absolute ${position} select-none cursor-pointer rounded-full bg-zinc-50 ring-2 ring-celadon-green p-3 ${animation} hover:animate-none`}
