...
const setup = () => {
  ...
  useEffect(() => {
    ...
    setWindowDimensions({ width: windowWidth, height: windowHeight });
    ...
    const animatedBgCanvas = document.getElementById('animated-bg-canvas');
    const context = animatedBgCanvas.getContext("2d");

    // Set canvas properties
    animatedBgCanvas.style.position = "absolute";
    animatedBgCanvas.style.top = "0px";
    animatedBgCanvas.style.left = "0px";
    animatedBgCanvas.width = windowWidth;
    animatedBgCanvas.height = windowHeight;
...
const AnimatedBackground = () => {
  ...
  return (
    <canvas id="animated-bg-canvas" className="max-h-full max-w-full -z-[99]"></canvas>
  );
}
